...
function Login_vs() {
	...
	var st = $("#loginip option:selected").val();  
	var count= parseInt(st, 10);
	if (count==10000){
		return;
	}
	hkvs[0]=arr[count][1];
	var iRet = WebVideoCtrl.I_Login(arr[count][1] , 1,  arr[count][2] ,  arr[count][3],  arr[count][4], {
		success: function (xmlDoc) {
			showOPInfo(" 登录成功！");
			setTimeout(function () {
				getChannel_vs(hkvs[0]);
			}, 10);
		},
		error: function () {
			showOPInfo(szIP + " 登录失败！");
...
function StartRealPlay_vs() {

	var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
		szIP = hkvs[0],
		iStreamType = parseInt("1", 10),
		iChannelID = parseInt(hkvs[1], 10),
		bZeroChannel =false,
		szInfo = "";
